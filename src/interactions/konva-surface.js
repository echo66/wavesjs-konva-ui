...
import KonvaWaveEvent from './konva-wave-event';
...
export default class Surface extends EventSource {
  ...
  _bindEvents() {
    ...
    const onDomDragStart = (e) => {
      let event = this._createEvent('dragstart', e);
      this.emit('event', event);
    };
    ...
    const onDomDragLeave = (e) => {
      let event = this._createEvent('dragleave', e);
      this.emit('event', event);
    };

    const onDomDragEnter = (e) => {
      let event = this._createEvent('dragenter', e);
      this.emit('event', event);
    };
